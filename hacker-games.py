...
def simulate_john_the_ripper():
    try:
        ...
        print(
            f"Cracking passwords using hash file: {hash_file} and wordlist: {wordlist}"
        )
        time.sleep(2)  # Simulate password cracking process
...
def simulate_distributed_denial_of_service():
    try:
        ...
        print(
            f"Initiating distributed denial of service attack on IP address: {ip_address} for {duration} seconds..."
        )
...
def simulate_data_exfiltration():
    try:
        ...
        print(
            f"Exfiltrating file: {file_path} to destination IP address: {destination_ip}"
        )
...
def simulate_oracle_operations():
    try:
        ...
        while True:
            ...
            file_choice = input(
                "Enter the number of the file to investigate or press Enter to exit: "
            )
            if not file_choice:
                break
            ...
            try:
                ...
                if 0 <= selected_index < len(case_files):
                    ...
                    if confirmation.lower() == "y":
                        ...
                        print(f"%> COMMAND: open file {file_path}")
                        print("File opened on Screen 5.")
                        time.sleep(3)  # Delay for 3 seconds
...
def progress_bar(start, end):
    ...
    for i in range(start, end + 1):
        print(
            f"Progress: {i}/{end} [{'#' * i + '-' * (end - i)}] {elapsed_time:.2f}/{total_time} seconds",
            end="\r",
            flush=True,
        )
...
def login_screen():
    ...
    while attempts < max_attempts and logged_in != True:
        ...
        if username in users and users[username] == password:
            ...
            clear()
            display_welcome_message()
        elif password.lower() in ("password", "passwort", "geheim", "sesame"):
            print("HaHa - nice try. Please try again!")
        else:
